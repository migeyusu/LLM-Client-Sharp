...
/// </summary>
public class MarkdownNode : RawNode<MarkdownNode, MarkdownText>
{
...
public class MarkdownText : BaseViewModel, IContentUnit
{
    ...
    public IList<ImageSource>? ImageSources
    {
        get => _imageSources;
        set
        {
    ...
    public MarkdownText(Block block, string rawMarkdownText, string markdownFilePath)
    {
        ...
        ConvertImages(_imageLinks);
    }

    private async void ConvertImages(IEnumerable<string> images)
    {
        ...
        foreach (var imageLink in images)
        {
            ...
            {
                Trace.TraceWarning("无法加载Markdown文件图像 {0}：{1}", imageLink, e);
            }
    ...
    private IList<ImageSource>? _imageSources;
    ...
    public async Task<IList<string>> GetImages(ILogger? logger)
    {
        if (_imageList == null)
        {
            ...
            foreach (var imageLink in _imageLinks)
            {
                ...
                {
                    logger?.LogWarning(e, "编码图像为base64时出错：{imageLink}", imageLink);
                }
            }
        }
