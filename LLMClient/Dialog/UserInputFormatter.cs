...
public class UserInputFormatter
{
        ...
        string? systemPrompt = null, CancellationToken token = default)
    {
        ...
        try
        {
            ...
            var promptAgent = new PromptBasedAgent(client)
            {
            ...
            var completedResult =
                    await promptAgent.SendRequestAsync(DefaultDialogContextBuilder.CreateFromHistory([new RequestViewItem(message)],
                        systemPrompt), token);
